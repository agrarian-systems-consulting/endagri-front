...
const ReadFicheLibrePage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      Axios(
        `http://51.210.14.158:3333/analyse/${id}/fiche-technique-libre/${id_ftl}`
      )
        .then((res) => {
  ...
  const addCoeffDepense = (coeff_depense) => {
    ...
    Axios.post(`http://51.210.14.158:3333/coeff_depense`, coeff_depense)
      .then((res) => {
  ...
  const deleteCoeffDepense = (id) => {
    Axios.delete(`http://51.210.14.158:3333/coeff_depense/${id}`)
      .then((res) => {
  ...
  const addCoeffVente = (coeff_vente) => {
    ...
    Axios.post(`http://51.210.14.158:3333/coeff_vente`, coeff_vente)
      .then((res) => {
        addToast('Le coefficient a bien été ajouté', {
          appearance: 'success',
          autoDismiss: true,
  ...
  const deleteCoeffVente = (id) => {
    Axios.delete(`http://51.210.14.158:3333/coeff_vente/${id}`)
      .then((res) => {
        addToast('Le coefficient a bien été supprimé', {
          appearance: 'success',
          autoDismiss: true,
